...
\subsection{Génération d'un exécutable via PythonInstaller}
Afin d'éviter d'utiliser le fichier python pour lancer l'interface graphique, on peut générer un exécutable qui le fera pour nous.\\
Pour cela on peut utiliser le paquet \textbf{pyinstaller}.\\
Il suffit de créer l'exécutable via la commande: \textbf{pyinstaller.exe --onefile --windowed IHM.py}\linebreak
Le nom de l'exécutable possèdera le même nom que le fichier python.\\
\textbf{onefile}: permet de réunir toute les librairies dont pyinstaller à besoin dans le .exe\\
\textbf{windowed}: permet d'indiquer si l'exécution se fais en mode interface graphique.
